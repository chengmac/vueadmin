<template>
    <div class="side-menu-wrapper">
        ...
        <Menu ref="menu" v-show="!collapsed" :active-name="activeName" :open-names="openedNames" :accordion="accordion" :theme="theme" width="auto" @on-select="handleSelect">
            <template v-for="(index, item) in menuList" name="index">
...
export default {
    ...
    methods: {
        ...
        getOpenedNamesByActiveName(name) {
            return this.router.matched.map(item => item.name).filter(item => item !== name);
        },
        updateOpenName(name) {
            if(name == 'home') {
                this.openedNames = [];
    ...
    computed: {
        textColor() {
            return this.theme === 'drak' ? '#fff' : '#495060';
        }
    },
    watch: {
        activeName(name) {
...
<style lang="less">
    @import '../../../style/components/side-menu.less';
</style>
